...
import { ShieldAlert, Clock, CheckCircle2, Gavel, Building2, Search, Bell, LogOut, Plus, LayoutDashboard, Star, Settings, FileText, ChevronDown, ArrowUpRight, Loader2, Timer, AlertTriangle, MessageSquare } from "lucide-react"
...
const INITIAL_MOCK_DATA = [
  {
    id: "CMP-7701",
    description: "Inappropriate comments and sexist slurs made by a faculty member during a lab session.",
    status: "Submitted",
    assignedRole: "hod",
    organization: "SYSTEM",
    priority: "high",
  ...
  {
    id: "CMP-7702",
    description: "Physical intimidation and catcalling reported near the girls' hostel entrance. Anonymous report filed.",
    status: "Under Review",
    assignedRole: "hod",
    organization: "SYSTEM",
    priority: "high",
  ...
  {
    id: "CMP-7704",
    description: "Systematic exclusion of LGBTQ+ students from the student body cultural fest planning.",
    status: "Submitted",
    assignedRole: "hod",
    organization: "SYSTEM",
    priority: "medium",
  ...
  {
    id: "CMP-7705",
    description: "Resolved: Formal mediation conducted for targeted bullying of an LGBTQ+ student in the computer science lab.",
    status: "Resolved",
    assignedRole: "hod",
    organization: "SYSTEM",
    priority: "high",
    ...
    userId: "user32",
    tags: ["Mediation", "Resolved"]
  },
  {
    id: "CMP-7706",
    description: "Suspicion of discriminatory grading practices reported against a senior lecturer.",
    status: "Under Review",
    assignedRole: "hod",
    organization: "SYSTEM",
    priority: "medium",
...
export default function HODDashboard() {
  ...
  const stats = useMemo(() => {
    const relevant = complaints.filter(c => c.assignedRole === "hod")
    return {
  ...
  const handleEscalate = (report: any) => {
    setComplaints(prev => prev.map(c => 
      c.id === report.id 
        ? { ...c, assignedRole: "safety", riskLevel: "Critical", status: "Under Review" } 
        : c
    ))
    toast({
      title: "Escalated to Safety Cell",
      description: `Case ${report.id} has been escalated for organization-wide safety review.`,
    })
  ...
  const handleResolve = (report: any) => {
    ...
    toast({
      title: "Case Resolved",
      description: `Disciplinary action for ${report.id} has been recorded.`,
    })
    setSelectedCase(null)
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Authenticating Command Authority...</p>
        </div>
      </div>
    )
  }
      ...
      <aside className="w-72 dashboard-sidebar text-white flex flex-col p-8 space-y-10">
        ...
        <div className="flex flex-col items-center py-6 space-y-3">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-white/50 uppercase tracking-widest">{session.role} Officer</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-12 overflow-y-auto space-y-12">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Badge className="status-badge-red text-white border-none px-3 py-1 rounded-full text-[10px] font-black">
          ...
          <div className="flex items-center gap-8">
            <div className="relative">
              ...
              <Input placeholder="Search something..." className="pl-11 h-12 w-64 rounded-2xl bg-white border-none shadow-sm" />
            </div>
        ...
        <section className="space-y-6">
          ...
          <p className="text-slate-400 text-lg">
            Direct oversight for critical violations <br /> at {session.organization} organization.
          </p>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <Card className="lg:col-span-8 border-none shadow-xl rounded-[3rem] p-10 bg-white">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-bold">Latest Actions</h3>
            ...
            <div className="space-y-6">
              {isDataLoading ? (
                 <div className="py-20 text-center text-slate-400 italic flex flex-col items-center gap-4">
                    ...
                    Accessing priority queue...
                 </div>
              ) : complaints.filter(c => c.assignedRole === "hod").length === 0 ? (
                <div className="py-20 text-center text-slate-400 italic">No critical incidents found.</div>
              ) : (
                complaints.filter(c => c.assignedRole === "hod").map((item) => {
                    ...
                    >
                      <div className="flex items-center gap-5">
                        <div className={cn(
                          ...
                          item.status === 'Resolved' ? 'bg-[#95b8a2]' :
                          item.riskLevel === 'Critical' ? 'bg-red-600' : 'bg-orange-500'
                        )}>
            ...
            {selectedCase && (
              <div className="flex flex-col h-[90vh]">
                <div className={cn(
                  ...
                  selectedCase.status === 'Resolved' ? 'bg-[#95b8a2]' :
                  selectedCase.riskLevel === 'Critical' ? 'bg-red-600' : 'bg-orange-500'
                )}>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <Badge className="bg-white/20 text-white border-none px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
                      Risk Analyzer: {selectedCase.riskLevel}
                    </Badge>
                    {selectedCase.status !== 'Resolved' && (
                      ...
                      >
                        ...
                        {isChatOpen ? "View Case Info" : "Ask query"}
                      </Button>
                    )}
                  </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                          ...
                          <Badge className={cn(
                            ...
                            selectedCase.riskLevel === 'Critical' ? "bg-red-500" :
                            selectedCase.riskLevel === 'High' ? "bg-orange-500" : "bg-yellow-500"
                          )}>
                        ...
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                          ...
                          <Badge variant="outline" className="px-3 py-1 rounded-lg font-bold border-primary text-primary">
                      ...
                      <div className="pt-6 border-t flex flex-col gap-3">
                        <div className="flex gap-3">
                          ...
                          >
                            ...
                            {selectedCase.status === 'Resolved' ? 'Resolved' : 'Execute Resolution'}
                          </Button>
                          <Button 
                            variant="outline"
                            className="flex-1 h-14 rounded-2xl font-bold border-2 border-slate-800 text-slate-800 hover:bg-slate-50"
                            onClick={() => handleEscalate(selectedCase)}
                            disabled={selectedCase.status === 'Resolved'}
                          >
                            <ArrowUpRight className="mr-2 h-5 w-5" />
                            Escalate to Safety Cell
                          </Button>
                        </div>
                      </div>
                    </div>
